...
{
	mParams = MSGPROXY->GetSection(SECTION_TASK)["reflector"];
	init();
	RsArg arg = MSGPROXY->GetSection(SECTION_TASK)["reflector_obsavoid"];
	mActionObsAvoid->SetArg(arg);
	mTryedTimes = 0;
	mPermitBackTimes = RsJson::GetInt(task, "back_times", -1,true);
	mFirstFailed = false;
	SetStatus("reflector");
}
...
void RsModeReflectorNew::Run()
{
	...
	{
		if (mActionReflector->GetState() == RsActionReflectorNew::STATE_ACHIEVED_DISTANCE)
		{
			if(!mFirstFailed)
			{
				mFirstFailed = true;
		...
		if (mActionReflector->GetState() == RsActionReflectorNew::STATE_REACHED_FINAL_DISTANCE)
		{
			SetStatus(std::string("finish reflection"));
			TaskRet(1);
			return;
		}
		else if (mActionReflector->GetState() == RsActionReflectorNew::STATE_FAILED)
		{
